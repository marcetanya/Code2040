...
class Node(object):
    ...
    def __init__(self, name, value):
        """Constructs the self.value and self.prev attributes of the class"""
        self.name = name
        self.cost = float('inf')
        self.value = value
        self.prev = None
...
def return_adj(point, flat_graph, length, width):
    ...
    if x < 1 and y < 1:
        return [flat_graph[point + 1], flat_graph[point + width], flat_graph[point + width +1]]

    elif x in range(1, width - 1) and y < 1:
        return [flat_graph[point - 1], flat_graph[point + 1], flat_graph[point + width - 1], flat_graph[point + width], flat_graph[point + width + 1]]
        
    elif x == width - 1 and y < 1:
        return [flat_graph[point - 1], flat_graph[point + width - 1], flat_graph[point + width]]

    elif x < 1 and y in range(1, length - 1):
        return [flat_graph[point - width], flat_graph[point - width + 1], flat_graph[point + 1], flat_graph[point + width], flat_graph[point + width + 1]]

    elif x in range(1, width - 1) and y in range(1, length - 1):
        return [flat_graph[point - width - 1], flat_graph[point - width], flat_graph[point - width + 1], flat_graph[point - 1], flat_graph[point + 1], flat_graph[point + width - 1], flat_graph[point + width], flat_graph[point + width + 1]]

    elif x == width - 1 and y in range(1, length - 1):
        return [flat_graph[point - width - 1], flat_graph[point - width], flat_graph[point - 1], flat_graph[point + width - 1], flat_graph[point + width]]
    
    elif x < 1 and y == length - 1:
        return [flat_graph[point - width], flat_graph[point - width + 1], flat_graph[point + 1]]
    
    elif x in range(1, width - 1) and y == length - 1:
        return [flat_graph[point - width - 1], flat_graph[point - width], flat_graph[point - width + 1], flat_graph[point - 1], flat_graph[point + 1]]

    elif x == width - 1 and y == length - 1:
        return [flat_graph[point - width - 1], flat_graph[point - width], flat_graph[point - 1]]
...
def wire_DHD_SG1(existingWires):
    ...
    start_node, goal_node = find_sg(flat_graph)
